...
4. Import this project into Vercel. Add the same `DATABASE_URL` and `DIRECT_URL` environment variables to Production and Preview as appropriate. Also add `CRON_SECRET` (a random string of at least 16 characters); Vercel sends it as an `Authorization` header when it runs the daily cleanup at `/api/maintenance`, and without it that route returns 401 and expired sessions are never removed. Vercel runs `npm run build` (`prisma generate`) on every deployment, so a cached dependency install cannot leave a stale Prisma Client behind. Set the runtime explicitly under **Settings → Build and Deployment → Node.js Version**.
